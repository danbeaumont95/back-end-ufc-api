module.exports = [
  {
    item_name: "UFC Unisex Hoodie with Built-In Face Cover - Black",
    price: 85,
    description:
      "The UFC custom PFD garment-dyed unisex pullover hoodie combines a built-in, loose-fitting tubular cowl jersey blend fabric sewn into the collar at the seam that doubles as a stylish face covering as needed.",
    sizes: "Small, Medium, Large",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCGMS0076_A_720x.jpg?v=1607108658",
    type: "Clothing",
  },
  {
    item_name: "UFC 5th Round Champion Snapback Flex Cap",
    price: 35,
    description:
      "Be a champion in your everyday life. We are bigger than the Octagon. We champion Champions. The Championship Gold Collection celebrates all of us.",
    sizes: "Small, Medium, Large",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/101030184-UFC5th-Round-Champion-Snapback-Flex-Cap-blk_B_900x.jpg?v=1614801167",
    type: "Accessories",
  },
  {
    item_name: "EA SPORTS UFC 4 PS4",
    price: 60,
    description:
      "Shape Your Legend in EA SPORTS UFC 4. In EA SPORTS UFC 4 the fighter you become is shaped by your fight style, your achievements, and your personality.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCGNV800000_720x.jpg?v=1597067809",
    type: "Gaming",
  },
  {
    item_name: "Conor McGregor Autographed and Framed Glove Shadowbox",
    price: 850,
    description:
      "This framed shadowbox collage consists of an official UFC fight glove autographed by Conor McGregor and a photograph of McGregor celebrating after he defeated Eddie Alvarez at UFC 205: Alvarez vs McGregor.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCMEQ009500-A_15d7ffc0-d26f-4539-8a4d-bc17fb8ee356_720x.jpg?v=1614357647",
    type: "Collectables",
  },
  {
    item_name: "UFC 257 Autographed Event Poster",
    price: 1000,
    description:
      "Commemorate the action of UFC 257 with this event poster. Featuring signatures from the event's participating athletes.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCMCL0277_900x.jpg?v=1613482227",
    type: "Collectables",
  },
  {
    item_name: "BMF Replica Belt",
    price: 850,
    description:
      "The BMF Replica Belt is a limited offer so don’t miss your chance to own a piece of UFC history. Photos of belt shown are of actual belt and not the replica belt offered.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCGCL0019_B_720x.jpg?v=1572641596",
    type: "Collectables",
  },
  {
    item_name: "UFC Camp Mug",
    price: 15,
    description: "Double wall stainless seteel is perfect for morning coffee.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCGBV001000_A_900x.jpg?v=1608325381",
    type: "Accessories",
  },
  {
    item_name:
      "Funko POP!: Ultimate Fighting Championship - Khabib Nurmagomedov",
    price: 15,
    description:
      "It was only a matter of time before Funko entered the Octagon. Collect all UFC POP Figurines with Georges St-Pierre, Khabib Nurmagomedov & UFC global superstar Conor McGregor.",
    sizes: "NA",
    img_url:
      "https://cdn.shopify.com/s/files/1/0035/4274/5152/products/UFCFNV0005_900x.jpg?v=1553271369",
    type: "Collectables",
  },
];
